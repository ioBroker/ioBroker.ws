...
function IOSocket(server, settings, adapter, ignore, store, checkUser) {
    if (!(this instanceof IOSocket)) {
        return new IOSocket(server, settings, adapter, ignore, store, checkUser);
    }

    socketio = require('iobroker.ws.server');
    ...
    this.unsubscribeAll = function () {
        if (this.server && this.server.sockets) {
            for (const s in this.server.sockets) {
    ...
    async function rename(_adapter, oldName, newName, options) {
        ...
        try {
            ...
            const files = await that.adapter.readDirAsync(_adapter, oldName, options);
        ...
        try {
            await that.adapter.renameAsync(_adapter, oldName, newName, options);
        } catch (error) {
            if (error.message !== 'Not exists') {
                throw error;
            }
    ...
    async function unlink(_adapter, name, options) {
        ...
        try {
            ...
            const files = await that.adapter.readDirAsync(_adapter, name, options);
            if (files && files.length) {
                for (let f = 0; f < files.length; f++) {
        ...
        try {
            await that.adapter.unlinkAsync(_adapter, name, options);
        } catch (error) {
            if (error.message !== 'Not exists') {
                throw error;
            }
    ...
    function socketEvents(socket, address, cb) {
        ...
        socket.on('log', function (text, level) {
            if (level === 'error') {
                that.adapter.log.error(text);
            } else if (level === 'warn') {
                that.adapter.log.warn(text);
            } else if (level === 'info') {
                that.adapter.log.info(text);
            } else {
                that.adapter.log.debug(text);
            }
        });
        ...
        if (that.settings.allowAdmin) {
            ...
            socket.on('requireLog', function (isEnabled, callback) {
                if (updateSession(socket) && checkPermissions(socket, 'setObject', callback)) {
                    ...
                    if (that.adapter.log.level === 'debug') {
                        showSubscribes(socket, 'log');
                    }
        ...
        socket.on('subscribe', function (pattern, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'subscribe', callback, pattern)) {
                ...
                if (that.adapter.log.level === 'debug') {
                    showSubscribes(socket, 'stateChange');
                }
                if (typeof callback === 'function') {
                    setImmediate(callback, null);
                }
        ...
        socket.on('unlink', function (_adapter, name, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'unlink', callback, name)) {
                unlink(_adapter, name, {user: socket._acl.user})
                    .then(() => callback && callback())
                    .catch(error => callback && callback(error));
            }
        });

        socket.on('deleteFile', function (_adapter, name, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'unlink', callback, name)) {
                that.adapter.unlink(_adapter, name, {user: socket._acl.user}, callback);
            }
        });

        socket.on('deleteFolder', function (_adapter, name, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'unlink', callback, name)) {
                unlink(_adapter, name, {user: socket._acl.user})
                    .then(() => callback && callback())
                    .catch(error => callback && callback(error));
        ...
        socket.on('rename', function (_adapter, oldName, newName, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'rename', callback, oldName)) {
                rename(_adapter, oldName, newName, {user: socket._acl.user})
                    .then(() => callback && callback())
                    .catch(error => callback && callback(error));
            }
        });
        ...
        if (!socket.conn) {
            ...
            socket.on('connect', function () {
                ...
                if (that.settings.apikey.includes('@pro_')) {
    ...
    function updateConnectedInfo() {
        ...
        if (that.server && that.server.sockets) {
            ...
            that.adapter.setState('info.connected', text, true);
        }
    }
    ...
    (function __constructor() {
        ...
        if (!that.settings.defaultUser.match(/^system\.user\./)) {
            that.settings.defaultUser = 'system.user.' + that.settings.defaultUser;
        }
