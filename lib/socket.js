...
function IOSocket(server, settings, adapter, ignore, store, checkUser) {
    ...
    async function rename(_adapter, oldName, newName, options) {
        ...
        try {
            ...
            if (files && files.length) {
                for (let f = 0; f < files.length; f++) {
                    await rename(_adapter, oldName + '/' + files[f], newName + '/' + files[f]);
                }
            }
        } catch (error) {
        ...
        return that.adapter.renameAsync(_adapter, oldName, newName, options);
    }
    ...
    async function unlink(_adapter, name, options) {
        ...
        try {
            ...
            if (files && files.length) {
                for (let f = 0; f < files.length; f++) {
                    await unlink(_adapter, name + '/' + files[f]);
                }
            }
        } catch (error) {
            // ignore, because it is a file and not a folder
        ...
        return that.adapter.unlinkAsync(_adapter, name, options);
    }
