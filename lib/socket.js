...
class IOSocket extends EventEmitter {
    constructor(server, settings, adapter, ignore, store, checkUser) {
        ...
        this.userKey    = 'connect.sid'; // const
    ...
    init(server) {
        ...
        if (this.settings.auth) {
            ...
            this._passportSocketIo = require('./passportSocket');
        }
        ...
        if (this.settings.auth && this.server) {
            this.server.use(this._passportSocketIo.authorize({
                checkUser:    this.checkUser,
                passport:     this._passport,
                cookieParser: this._cookieParser,
                key:          this.userKey,             // the name of the cookie where express/connect stores its session_id
            ...
            this.adapter.checkPassword(user, pass, res => {
                if (res) {
                    ...
                    if (typeof callback === 'function') {
                        ...
                        this.adapter.log.warn('[this._getUserFromSocket] Invalid callback');
                    }
                } else {
    ...
    static getClientAddress(socket) {
        let address;
            ...
            if (this.settings.auth) {
                this._getUserFromSocket(socket, (err, user) => {
                    if (err || !user) {
                        ...
                        this.adapter.calculatePermissions(user, commandsPermissions, acl => {
                            const address = IOSocket.getClientAddress(socket);
                            // socket._acl = acl;
                ...
                this.adapter.calculatePermissions(this.settings.defaultUser, commandsPermissions, acl => {
                    const address = IOSocket.getClientAddress(socket);
                    // socket._acl = acl;
                    socket._acl = IOSocket.mergeACLs(address, acl, this.settings.whiteListSettings);
                    this.socketEvents(socket, address, cb);
            ...
            if (commandsPermissions[command]) {
                ...
                if (commandsPermissions[command].type) {
                    if (socket._acl[commandsPermissions[command].type] &&
                        ...
                        this.adapter.log.warn(`No permission for "${socket._acl.user}" to call ${command}. Need "${commandsPermissions[command].type}"."${commandsPermissions[command].operation}"`);
                    }
                } else {
                    return true;
                }
    ...
    socketEvents(socket, address, cb) {
        ...
        if (this.settings.allowAdmin) {
            ...
            // function uses this, so don't use lambda
        ...
        socket.on('disconnect', error => {
            this.adapter.log.info(`<== Disconnect ${socket._acl.user} from ${IOSocket.getClientAddress(socket)} ${socket._name || ''}`);
            this._unsubscribeSocket(socket, 'stateChange');
            this._unsubscribeSocket(socket, 'objectChange');
            this._unsubscribeSocket(socket, 'log');
            this.infoTimeout = this.infoTimeout || setTimeout(() => {this.infoTimeout = null; this.updateConnectedInfo()}, 1000);
        ...
        if (!socket.conn) {
            ...
            // if server mode
            if (socket._sessionID && this.adapter.config.auth) {
                socket._secure    = true;
