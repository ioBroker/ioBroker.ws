...
class SocketCommands {
    ...
    __initCommandsObjects() {
        ...
        this.commands['clientSubscribe'] = (socket, targetInstance, messageType, data, callback) => {
            ...
            // @param {string} messageType - message type, e.g. "startRecording"
            // @param {string} data - optional data object, e.g. {width: 640, height: 480}
            // @param {function} callback - `function (error, result)`, target instance MUST acknowledge the subscription and returns some object as result
            ...
            if (!this.clientSubscribes[sid][targetInstance].includes(messageType)) {
                ...
                this.adapter.sendTo(targetInstance, 'clientSubscribe', {type: messageType, sid, data, provider: this.adapter.namespace}, result =>
        ...
        this.commands['clientUnsubscribe'] = (socket, targetInstance, messageType, callback) => {
            ...
            // @param {string} targetInstance - instance name, e.g. "cameras.0"
            // @param {string} messageType - message type, e.g. "startRecording"
            // @param {function} callback - `function (error, notSubscribed)`, target instance MUST NOT acknowledge the un-subscription
            const sid = socket.id;
            ...
            if (!this.clientSubscribes[sid][targetInstance].includes(messageType)) {
                ...
                this.adapter.sendTo(targetInstance, 'clientUnsubscribe', {type: [messageType], sid, provider: this.adapter.namespace});
