...
class SocketWS extends SocketCommon {
    ...
    _waitForSessionEnd(socket) {
        ...
        this.store && this.store.get(sessionId, (err, obj) => {
            if (obj) {
                const expires = new Date(obj.cookie.expires);
                const interval = expires.getTime() - Date.now();
                if (interval > 0) {
                    socket._sessionTimer = socket._sessionTimer || setTimeout(() => this._waitForSessionEnd(socket), interval > 3600000 ? 3600000 : interval);
